...
import {
  ...
  BookingReserveRequestDto,
  BookingReserveResponseDto,
  BookingService,
  BookingSlotDto,
...
export class MeetingInviteComponent implements OnInit {
  ...
  bookingApiService = inject(BookingService);
  ...
  sendRequest() {
    ...
    const { zone } = this.formGroup.value;

    if (!this.selectedSlotStartUtc) {
      console.error('A valid time slot is required to create a booking.');
      return;
    }

    const timezoneLabel = zone ?? this.timezoneOptions[0];
    const timezone = this.extractIanaTimezone(timezoneLabel);
    const selectedSlot = this.availableTimesOfSelectedDay.find(
      (slot) => slot.startUtc === this.selectedSlotStartUtc,
    );

    const request: BookingReserveRequestDto = {
      startUtc: this.selectedSlotStartUtc,
      durationMinutes: selectedSlot?.durationMinutes,
      timezone,
      name: this.formGroup.value.name ?? '',
      email: this.formGroup.value.email ?? '',
      phone: this.formGroup.value.phone ?? '',
      subject: this.formGroup.value.subject ?? 'Meeting',
      comments: this.formGroup.value.comments ?? '',
    };

    this.bookingApiService
      .bookingControllerReserve(request)
      .subscribe({
        next: (response: BookingReserveResponseDto) => {
          console.log('Booking created successfully', response);
          const selectedDate = new Date(this.selectedSlotStartUtc as string);
          ...
          const timeLabel =
            this.availableTimesOfSelectedDay.find(
              (slot) => slot.startUtc === this.selectedSlotStartUtc,
            )?.value ?? '';
          ...
          this.router.navigate(['thanks'], {
            queryParams: {
              name: this.formGroup.value.name,
              message: `I am looking forward to our time on ${dateLabel} at ${timeLabel} ${timezoneLabel}!`,
            },
          });
        },
        error: (error: unknown) => {
